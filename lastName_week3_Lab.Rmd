...
```{r, "Step 2"}
# Write your code below.
boxplots <- ggplot(mtcars, aes(mpg)) +
  geom_boxplot() +
  facet_wrap(~cyl) +
  labs(title = "Step 2 - Boxplots") +
...
```{r, "Step 3"}
# Write your code below.
multiline <- ggplot(mtcars, aes(x = wt, y = mpg, group = am, color = as.factor(am))) +
  geom_line() +
  labs(title = "Step 3 - Multiline Chart") +
  xlab("Weight") +
  ylab("MPG") +
  guides(color=guide_legend("AM"))
...
barchart <- ggplot(mtcars, aes(x = carnames, y = wt)) +
  geom_bar(stat = "identity") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "Step 4 - Barchart") +
  xlab("Car") +
  ylab("Weight")
...
```{r, "Step 5"}
# Write your code below.
scatter <- ggplot(mtcars, aes(x = mpg, y = wt, size = qsec, color = qsec)) +
  geom_point() +
  labs(title = "Step 5 - Scatter Chart") +
  xlab("MPG") +
  ylab("Weight") +
  guides(color = guide_legend("Quarter Second")) +
  guides(size = guide_legend("Quarter Second"))
